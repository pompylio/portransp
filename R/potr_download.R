...
#' (yyyymm, yyyymmdd). Depends on the entrance requirements of the 'opendata'.
...
#'
#' @examples
#' potr_download(opendata = 19, reference = 2018)
#' potr_download(opendata = 19, reference = 2018, destfile = "~/data")
#' potr_download(opendata = 19, reference = 2018, destfile = "~/data", filename = "orcamento.zip")
#' potr_download(opendata = "19", reference = "2018")
#' potr_download(opendata = "19", reference = "2018", destfile = "~/data")
#' potr_download(opendata = "19", reference = "2018", destfile = "~/data", filename = "orcamento.zip")
...
potr_download <- function(opendata, reference, destfile, filename, download.file.mode, ...) {
  ...
  if (!nchar(reference) == nchar(potrdt$formatdate))
    stop("Valid input to 'opendata' and 'reference': ", potrms, call. = FALSE)
  if (missing(filename)) filename <- paste0(potrdt$dataset, "_", reference, ".zip")
